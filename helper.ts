...
export async function verifyFlightParts(
  selectedfrom: string,
  selectedTo: string,
  index: number = 0
) {
  let first = true;
  let last = "";
  let orignElements = await $$('div[data-element-name="flight-detail-button"]')[
    index
  ].$$('p[data-testid="origin-airport"]');
  for (const airport of orignElements) {
    await airport.waitForClickable({
      timeout: 5000,
      timeoutMsg: "From Airport not found in parts",
    });
