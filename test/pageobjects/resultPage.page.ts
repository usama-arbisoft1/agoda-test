...
class resultPage {
  ...
  private get fromDestInput() {
    return $(
      'div[data-testid="flight-origin-text-search"] input[placeholder="Flying from"]'
    );
  }
  private get toDestInput() {
    return $(
      'div[data-testid="flight-destination-text-search"] input[placeholder="Flying to"]'
    );
  }
  private get thirdDivText() {
    return $(
      'div[data-testid="flex-date-item-card"].a2578-border-product-primary-strong div p'
    );
  }
  ...
  private async verifySingleDestination(
    airport: string,
  ...
  private async verifyBestResellerOption() {
    ...
    await expect(this.bestOverallButton).toHaveElementClass(
      "a2578-text-product-primary",
      { message: "Best Overall isnt selected by default!" }
    );
  }
  ...
  private async verifyOrigin(originShortCode: string) {
    await this.originCard.waitForDisplayed({
      timeout: 5000,
      timeoutMsg: "Orign Text not available",
    });
  ...
  private async verifyDestinations(destinationShortCode: string) {
    await this.destinationCard.waitForDisplayed({
      timeout: 5000,
      timeoutMsg: "Destination Text not available",
    });
    await expect(await this.destinationCard.getText()).toEqual(
  ...
  private async verifyTime() {
    ...
    let timeElements = await this.flightDetailButton.$$(
    ...
    let layOverTimes = await this.flightDetailButton.$$(
      'div[data-testid="layover"] p'
    );
  ...
  public async VerifyResultPageData(
    from: string,
    to: string,
