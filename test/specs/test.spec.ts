...
describe("Agoda Flight Booking Feature", () => {
  it("Search a flight", async () => {
    addDescription(
      "Testing Flight Searching Feature For Agoda.",
      TYPE.MARKDOWN
    );
    addSeverity("testing");
    ...
    await step("Search Flight", async () => {
      SearchPageData = await homePage.searchFlight(
        origin,
        destination,
        2,
        2,
    ...
    await step("Verify Flight Search Results", async () => {
      await resultPage.VerifyResultPageData(
        origin,
        destination,
        SearchPageData.airportShortcodes.origin,
        SearchPageData.airportShortcodes.destination,
        SearchPageData.datesDate.departureDate,
        SearchPageData.datesDate.returnDate
      );
    });
